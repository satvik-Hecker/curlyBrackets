...
export default function RoadmapPage() {
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      ...
      data.forEach((entry) => {
        map[entry.topic_srn] = (map[entry.topic_srn] || 0) + 1;
      });
  ...
  return (
    ...
    <div className="min-h-screen w-full relative bg-black font-mono">
      ...
      <div className="flex flex-col items-center px-6 md:px-8 mt-12 md:mt-20">
        ...
        >
          <h1 className="text-xl md:text-3xl lg:text-4xl text-white leading-tight drop-shadow-2xl">
            Learn Development now,
            <br />
          ...
          >
            <AnimatePresence mode="wait">
              {visibleTopics.map((topic, index) => {
                const totalSubtopics = subtopics[topic.srn as keyof typeof subtopics]?.length || 0;
                const completed = progressMap[topic.srn] || 0;
                const total = subtopics[topic.srn as keyof typeof subtopics]?.length || 0;
                const percent = total > 0 ? Math.round((completed/total)*100) : 0;


                let label = "Start Learning";
                if (completed > 0 && completed < totalSubtopics) label = "Continue Learning";
                if (completed === totalSubtopics && totalSubtopics > 0) label = "Revise";
                          ...
                          <div className="space-y-4 flex-1 flex flex-col">
                            ...
                            <div className="flex-shrink-0">
                              <div className="flex items-center justify-between mb-3">
                                <span className={`px-3 py-1 rounded-full text-xs font-bold ${getDifficultyColor(topic.difficulty)}`}>
                                  {topic.difficulty}
                                </span>
                              ...
                              <div className="flex flex-wrap gap-1 mb-4">
                                {topic.tags.slice(0, 3).map((tag, tagIndex) => (
                                  <span key={tagIndex} className="px-2 py-1 bg-teal-100 text-teal-700 text-xs rounded-md font-medium shadow-sm">
                                    {tag}
                                  </span>
                                ))}
                              </div>
                            ...
                            <div className="mt-4">
                                  <div className="flex justify-between text-xs text-shadow-gray-600 mb-1">
                                    <span>{completed}/{total} completed</span>
                                    <span>{percent}%</span>
                                  </div>
                                  <Progress value={percent} className="h-2 bg-teal-100"></Progress>
                                </div>
        ...
        >
          {Array.from({ length: maxIndex + 1 }).map((_, index) => (
            <button
              key={index}
              className={`h-2 rounded-full transition-all duration-300 shadow-sm ${index === currIndex ? "bg-teal-300 w-8 shadow-teal-300/50" : "bg-white/40 w-2 hover:bg-white/60"}`}
